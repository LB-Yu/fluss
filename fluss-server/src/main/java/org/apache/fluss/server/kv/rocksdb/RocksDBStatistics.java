...
public class RocksDBStatistics implements AutoCloseable {
    ...
    /**
     * Get block cache usage directly from the Cache object.
     *
     * @return block cache usage in bytes, or 0 if not available
     */
    public long getBlockCacheUsage() {
        try (ResourceGuard.Lease lease = resourceGuard.acquireResource()) {
            ...
            LOG.debug("Failed to get block cache usage from RocksDB", e);
        }
        return 0L;
    }
